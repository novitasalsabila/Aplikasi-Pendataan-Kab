<x-app-layout>
    <div class="max-w-3xl mx-auto p-6 bg-white dark:bg-gray-800 shadow-lg rounded-lg">
        <h2 class="text-2xl font-semibold mb-6 text-gray-800 dark:text-gray-100">
            ➕ Tambah Data OPD
        </h2>

        <form action="{{ route('departments.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label class="block font-semibold mb-1 text-gray-700 dark:text-gray-300">Nama OPD</label>
                <input type="text" name="name" required
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                <label class="block font-semibold mb-1 text-gray-700 dark:text-gray-300">Email</label>
                <input type="email" name="email"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                <label class="block font-semibold mb-1 text-gray-700 dark:text-gray-300">Nama Kepala OPD</label>
                <input type="text" name="head_name"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                <label class="block font-semibold mb-1 text-gray-700 dark:text-gray-300">No. HP Kepala OPD</label>
                <input type="text" name="head_phone"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
            </div>

            <div class="flex items-center justify-end space-x-3">
                <a href="{{ route('departments.index') }}"
                    class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition">Kembali</a>
                <button type="submit"
    class="bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-black px-4 py-2 rounded-lg transition">
    Simpan
</button>

            </div>
        </form>
    </div>
</x-app-layout>
